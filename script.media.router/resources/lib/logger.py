...
class logger:
    ...
    def log(message, level=xbmc.LOGDEBUG):
        ...
        if isinstance(message, bytes):
            # Python 2 str (bytes) — decode to unicode first
            message = message.decode("utf-8")
        elif text_type is not str and isinstance(message, text_type):
            # Python 2 unicode — encode to utf-8 bytes for xbmc.log
            message = message.encode("utf-8")
            prefix = logger.log_message_prefix.encode("utf-8")
        # Python 3: message is already str, pass through unchanged
