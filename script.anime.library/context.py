...
def run():
    ...
    logger.info("context: kodi_dir_name=%r run_mode=%r" % (kodi_dir_name, run_mode))

    anime_sources = get_anime_sources(kodi_dir_name)

    if not anime_sources:
        logger.warning("context: no anime sources found in Kodi video sources")
        xbmcgui.Dialog().ok(
    ...
    for anime_root, kodi_flat in anime_sources:
        logger.info("context: rebuilding symlinks for %r" % anime_root)
        try:
    ...
    if run_mode == RUN_MODE_REINDEX_AND_SCAN:
        ...
        logger.info("context: skipping scan (reindex only mode)")
